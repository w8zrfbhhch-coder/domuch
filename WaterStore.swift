...
struct TodoItem: Identifiable, Hashable {
    enum Link: Hashable {
        case none, water, protein, coffee, carbs
...
enum TrackerKind: String, Codable, CaseIterable, Identifiable {
    case todo, water, protein, coffee, carbs
    ...
    /// tap each. Carbs uses the same three-tier shape as protein/
    /// water, sized to common food-serving increments (a slice of
    /// bread ≈15g, a cup of rice ≈45g, rounded to 15/30/60).
    var presets: [Int] {
        switch self {
        case .todo: []
...
struct AppData: Codable {
    ...
    /// by default; coffee/carbs are newer and start off, an explicit
    /// "add" away, rather than suddenly appearing active for existing
    /// users (see the matching decode fallback below).
    var disabledTrackers: Set<TrackerKind> = [.coffee, .carbs]
    ...
    init(from decoder: Decoder) throws {
        ...
        carbsGoalG = try c.decodeIfPresent(Int.self, forKey: .carbsGoalG) ?? 250
        ...
        // default THOSE two off rather than []; an explicit (even
        // empty) saved set is always respected as-is.
        disabledTrackers = try c.decodeIfPresent(Set<TrackerKind>.self, forKey: .disabledTrackers) ?? [.coffee, .carbs]
...
final class WaterStore: ObservableObject {
    ...
    var coffeeProgress: Double {
        min(1.0, Double(todayCoffeeTotal) / Double(max(data.coffeeGoal, 1)))
    }
    ...
    var carbsProgress: Double {
        min(1.0, Double(todayCarbsTotal) / Double(max(data.carbsGoalG, 1)))
    }
    ...
    func todoLabel(for item: TodoItem) -> String {
        ...
        case .carbs:
            let roundedG = Int((Double(data.carbsGoalG) / 25).rounded() * 25)
            return "\(roundedG)g carbs"
        }
    }
    ...
    #if DEBUG
    /// Wipes EVERY water/protein/coffee/carbs/to-do entry ever
    /// logged — real history included, not just today and not just
    /// seeded demo data — so today becomes a genuine day one. Goals
    /// and custom to-do slot names are settings, not history, and
    /// are left alone. Deletes the CloudKit copies too. Debug-only
    /// and irreversible — never compiled into a release build, and
    /// there's no confirmation dialog here (the caller adds one), so
    /// only wire this up somewhere you won't tap by accident.
    ...
    var dailyTotals: [DailyTotal] {
        ...
        let days = Set(waterByDay.keys).union(proteinByDay.keys).union(coffeeByDay.keys).union(carbsByDay.keys).union(todoByDay.keys)
        return days
    ...
    private func pullFromCloudKit() async {
        ...
        let (w, p, c, cb, t, s) = await (water, protein, coffee, carbs, todos, settings)
